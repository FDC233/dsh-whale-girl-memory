	...
	factory: (require) => {
		...
		const copy = {
			"zh-CN": {
				tab: "记忆学习",
				title: "🧠 鲸鱼娘记忆学习",
				subtitle: "跨会话长期记忆 · 自动学习习惯/事实/经验 · 规范化 JSON 经验文件 · 支持导入他人经验",
				loading: "正在读取记忆库…",
				total: "共",
				totalUnit: "条记忆",
				types: { preference: "习惯偏好", fact: "关键事实", lesson: "经验教训", workflow: "流程技巧", "tool-usage": "工具使用" },
				all: "全部",
				searchPh: "搜索标题/正文/标签…",
				useCount: "使用",
				times: "次",
...
`;
		const TYPE_ORDER = ["preference", "fact", "lesson", "workflow", "tool-usage"];
			...
			return h("div", { className: "mem-page" },
				...
				ready ? h("div", { className: "mem-card" },
					h("div", { className: "mem-summary" },
						h("span", { className: "mem-chip" }, `${t.total} ${state.status.total} ${t.totalUnit}`),
						[["", t.all], ...TYPE_ORDER.map((ty) => [ty, t.types[ty] ?? ty])].map(([ty, label]) =>
							h("span", { key: ty || "all", className: `mem-chip${typeFilter === ty ? " on" : ""}`, onClick: () => setTypeFilter(ty) }, `${label} (${ty ? (counts[ty] || 0) : state.status.total})`)
						)
					),
				...
				h("div", { className: "mem-card" },
					...
					h("div", { className: "mem-row" },
						h("select", { className: "mem-select", value: form.type, onChange: (e) => setForm({ ...form, type: e.target.value }) },
							TYPE_ORDER.map((ty) => h("option", { key: ty, value: ty }, t.types[ty] ?? ty))
						),
